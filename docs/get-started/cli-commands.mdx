...
This section details the present commands, command flags in the Fabric Edge, and how they're used.
...
Used to specify the data directory used for storing Fabric Edge client data. Default: `./test-chain`.
...
<Tabs>
  ...
  <TabItem value="example" label="Example">
    ...
    server --access-control-allow-origins "https://fabric-edge-docs.techaroha.com"
...
Sets the directory for the Fabric Edge genesis data. Default: `./genesis.json`.
...
| secrets generate         | Generates a secrets manager configuration file which can be parsed by the Fabric Edge    |
...
Sets the directory for the Fabric Edge data if the local FS is used.
...
Specifies the name of the node for on-service record keeping. Default: `fabric-edge-node`
