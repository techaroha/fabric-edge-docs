...
The below guide will instruct you on how to set up a Fabric Edge network on your local machine for testing and development
purposes.

The procedure differs greatly from the way you would want to set up Fabric Edge network for a real use scenario on
...
````bash
fabric-edge secrets init --data-dir test-chain-1
````

````bash
fabric-edge secrets init --data-dir test-chain-2
````

````bash
fabric-edge secrets init --data-dir test-chain-3
````

````bash
fabric-edge secrets init --data-dir test-chain-4
````
...
`bootnode` is not a special instance of the fabric-edge node. Every fabric-edge node can serve as a `bootnode`, but
...
And lastly, we need the `<node_id>` which we can get from the output of the previously ran command `fabric-edge secrets init --data-dir test-chain-1` command (which was used to generate keys and data directories for the `node1`)
...
````bash
fabric-edge genesis --consensus ibft --ibft-validators-prefix-path test-chain- --bootnode /ip4/127.0.0.1/tcp/10001/p2p/16Uiu2HAmJxxH1tScDX2rLGSU9exnuvZKNM9SoK3v315azp68DLPW --bootnode /ip4/127.0.0.1/tcp/20001/p2p/16Uiu2HAmS9Nq4QAaEiogE4ieJFUYsoH28magT7wSvJPpfUGBj3Hq 
````

What this command does:

* The `--ibft-validators-prefix-path` sets the prefix folder path to the one specified which IBFT in Fabric Edge can
...
Because we are attempting to run a Fabric Edge network consisting of 4 nodes all on the same machine, we need to take care to 
...
````bash
fabric-edge server --data-dir ./test-chain-1 --chain genesis.json --grpc-address :10000 --libp2p :10001 --jsonrpc :10002 --seal
````

To run the **second** client:

````bash
fabric-edge server --data-dir ./test-chain-2 --chain genesis.json --grpc-address :20000 --libp2p :20001 --jsonrpc :20002 --seal
````

To run the **third** client:

````bash
fabric-edge server --data-dir ./test-chain-3 --chain genesis.json --grpc-address :30000 --libp2p :30001 --jsonrpc :30002 --seal
````

To run the **fourth** client:

````bash
fabric-edge server --data-dir ./test-chain-4 --chain genesis.json --grpc-address :40000 --libp2p :40001 --jsonrpc :40002 --seal
````
...
````bash 
fabric-edge server --data-dir <directory_path> --chain <genesis_filename> --grpc-address <portNo> --libp2p <portNo> --jsonrpc <portNo>
````
For example, you can add **fifth** Non-validator client by executing the following command :

````bash
fabric-edge server --data-dir ./test-chain --chain genesis.json --grpc-address :50000 --libp2p :50001 --jsonrpc :50002 
````
:::
...
## Step 5: Interact with the fabric-edge network
