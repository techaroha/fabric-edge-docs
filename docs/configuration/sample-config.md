...
The command used to start the server with a config file: `fabric-edge server --config <config_file_name>`

## Export config file with default configuration
The configuration with default settings for the Fabric Edge server can be exported into a config file in either `yaml` or `json` file format.
...
```bash
fabric-edge server export --type yaml
```
or just
```bash
fabric-edge server export
```
...
```bash
fabric-edge server export --type json
```
